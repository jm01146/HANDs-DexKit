#include <SPI.h>
#include <avr/pgmspace.h>

const unsigned short firmware_length = 4094;

const unsigned char firmware_data[] PROGMEM = {
 };


#define CPI 1000 //Default CPI


// SROM commands
#define SROM_INIT            0x1D
#define SROM_START_DOWNLOAD  0x18
#define SROM_Load_Burst 0x62
#define WRITE_BIT 0x80
#define READ_BIT 0x7f
#define REG_SROM_ENABLE 0x13
#define raw_data_burst 0x64
#define cmd_reset_val 0x5A


//Product Registers
#define ProductID 0x00
#define RevisionID 0x01
#define Observation 0x24

//Motion Registers
#define motionSensor 0x02
#define motion_burst 0x50
#define Delta_X_L 0x03
#define Delta_X_H 0x04
#define Delta_Y_L 0x05
#define Delta_Y_H 0x06

//Resolution
#define resolution_low 0x0E
#define resolution_high 0x0F
#define config_2 0x10
#define angle_tune 0x11
#define angle_snap 0x42
#define lift_config 0x63

//Power
#define run_downshift 0x14
#define reset 0x3A
#define shutdown 0x3B


// Firmware Variables (Utilized for uploading)
extern const unsigned short firmware_length;
extern const unsigned char firmware_data[];

/*
CS or SS Pin and Motion Pin defined
Implicit Pins:
MISO = 50
MOSI = 51
SCK = 52

Voltage Pin at 3.3V (This sensor has an onboard regulator)

GND at any of the two GND pins (Arduino MEGA)

*/ 

const int chipSelectPin = 53;
const int motion = 2;

const int outputX = 5;
const int outputY = 6;

const int NUMCPI = 7;

/* 
This is currently unused. This could be utilized for an automatic button transition among CPIs, but for the moment
we can simply define CPI as 1000 (see definitions above) and change that value accordingly.

unsigned long Cpis[NUMCPI] = { 400, 600, 800, 1000, 1200, 1400, 1600 };
struct CpiUpdater {
  bool target_set;
  bool updated;
  uint8_t target_cpi_index;
};


CpiUpdater CpiUpdate = {false, false, 6};
*/

// Motion result returned by the PMW3389.
struct MotionData {
  bool motionDetected;
  bool onSurface;
  int16_t dx;
  int16_t dy;
};

const SPISettings PMW3389_SPI(2000000, MSBFIRST, SPI_MODE3);

int32_t machineXCounts = 0;
int32_t machineYCounts = 0;


// Write one PMW3389 register.
//
// PMW3389 timing:
//   - Keep NCS low for at least 35 us after a write.
//   - Allow about 180 us total before the next SPI command.
void writeRegister(uint8_t reg_addr, uint8_t data)
{
  SPI.beginTransaction(PMW3389_SPI);

  digitalWrite(chipSelectPin, LOW);
  delayMicroseconds(1);

  SPI.transfer(reg_addr | WRITE_BIT);
  SPI.transfer(data);

  delayMicroseconds(35);
  digitalWrite(chipSelectPin, HIGH);

  SPI.endTransaction();

  delayMicroseconds(145);
}


// Read one normal PMW3389 register.
//
// Normal register reads require a 160 us address-to-data delay.
// The shorter 35 us delay is only for Motion_Burst reads.
uint8_t readRegister(uint8_t reg_addr)
{
  SPI.beginTransaction(PMW3389_SPI);

  digitalWrite(chipSelectPin, LOW);
  delayMicroseconds(1);

  SPI.transfer(reg_addr & READ_BIT);
  delayMicroseconds(160);

  uint8_t data = SPI.transfer(0x00);

  delayMicroseconds(1);
  digitalWrite(chipSelectPin, HIGH);

  SPI.endTransaction();

  delayMicroseconds(19);
  return data;
}


// Upload the 4094-byte SROM firmware already included above.
void uploadFirmware()
{
  // Disable rest mode before loading firmware.
  writeRegister(config_2, 0x00);

  writeRegister(REG_SROM_ENABLE, SROM_INIT);
  delay(10);

  writeRegister(REG_SROM_ENABLE, SROM_START_DOWNLOAD);

  // Start the download immediately after the 0x18 command.
  SPI.beginTransaction(PMW3389_SPI);

  digitalWrite(chipSelectPin, LOW);
  delayMicroseconds(1);

  SPI.transfer(SROM_Load_Burst | WRITE_BIT);
  delayMicroseconds(15);

  for (uint16_t i = 0; i < firmware_length; i++) {
    SPI.transfer(pgm_read_byte(&firmware_data[i]));
    delayMicroseconds(15);
  }

  delayMicroseconds(200);
  digitalWrite(chipSelectPin, HIGH);

  SPI.endTransaction();

  delayMicroseconds(200);
}


// PMW3389 resolution registers use 50-CPI steps.
// Register value 0 means 50 CPI, 19 means 1000 CPI, etc.
void setCPI(uint16_t cpi)
{
  uint16_t cpival;

  if (cpi <= 50) {
    cpival = 0;
  } else {
    cpival = (cpi / 50) - 1;
  }

  if (cpival > 319) {
    cpival = 319; // 16000 CPI maximum
  }

  // Write high byte first, then low byte.
  writeRegister(resolution_high, (cpival >> 8) & 0xFF);
  writeRegister(resolution_low, cpival & 0xFF);
}


// Read relative X/Y movement from the Motion_Burst register.
MotionData readMotionBurst()
{
  MotionData result = {false, false, 0, 0};

  SPI.beginTransaction(PMW3389_SPI);

  digitalWrite(chipSelectPin, LOW);
  delayMicroseconds(1);

  SPI.transfer(motion_burst);
  delayMicroseconds(35);

  uint8_t motionStatus = SPI.transfer(0x00);
  uint8_t observation  = SPI.transfer(0x00);
  uint8_t deltaXL      = SPI.transfer(0x00);
  uint8_t deltaXH      = SPI.transfer(0x00);
  uint8_t deltaYL      = SPI.transfer(0x00);
  uint8_t deltaYH      = SPI.transfer(0x00);

  delayMicroseconds(1);
  digitalWrite(chipSelectPin, HIGH);

  SPI.endTransaction();

  result.motionDetected = (motionStatus & 0x80) != 0;
  result.onSurface      = (motionStatus & 0x08) == 0;

  // Combine the low and high bytes as an unsigned word first,
  // then reinterpret the complete word as a signed 16-bit value.
  result.dx = (int16_t)(((uint16_t)deltaXH << 8) | deltaXL);
  result.dy = (int16_t)(((uint16_t)deltaYH << 8) | deltaYL);

  (void)observation;
  return result;
}


void setup()
{
  Serial.begin(115200);

  pinMode(outputX, OUTPUT);
  pinMode(outputY, OUTPUT);
  pinMode(motion, INPUT_PULLUP);
  pinMode(chipSelectPin, OUTPUT);

  digitalWrite(chipSelectPin, HIGH);

  SPI.begin();
  delay(10);

  // Reset the SPI port state by toggling NCS.
  digitalWrite(chipSelectPin, LOW);
  delayMicroseconds(40);
  digitalWrite(chipSelectPin, HIGH);
  delayMicroseconds(40);

  // Power-up reset.
  writeRegister(reset, cmd_reset_val);
  delay(50);

  // The startup sequence requires these registers to be read and discarded.
  (void)readRegister(motionSensor);
  (void)readRegister(Delta_X_L);
  (void)readRegister(Delta_X_H);
  (void)readRegister(Delta_Y_L);
  (void)readRegister(Delta_Y_H);

  uploadFirmware();

  // SROM_ID must be the first register read after the firmware download.
  uint8_t sromId = readRegister(0x2A);

  writeRegister(config_2, 0x00);
  setCPI(CPI);

  uint8_t productId = readRegister(ProductID);
  uint8_t inverseId = readRegister(0x3F);

  Serial.println(F("PMW3389 startup"));
  Serial.print(F("Product ID: 0x"));
  Serial.println(productId, HEX);
  Serial.print(F("Inverse ID: 0x"));
  Serial.println(inverseId, HEX);
  Serial.print(F("SROM ID: 0x"));
  Serial.println(sromId, HEX);

  // Explicitly initialize motion-burst mode.
  writeRegister(motion_burst, 0x00);

  //Serial.println(F("dX,dY,X_counts,Y_counts,X_mm,Y_mm"));
}


void loop()
{
  static unsigned long lastTransmitTime = 0;

  MotionData m = readMotionBurst();

  // The PMW3389 reports relative movement. Accumulating the deltas creates
  // a machine-style position estimate.
  if (m.motionDetected && m.onSurface) {
    machineXCounts += m.dx;
    machineYCounts += m.dy;
  }

  const float xMM = machineXCounts * (25.4f / CPI);
  const float yMM = machineYCounts * (25.4f / CPI);

  if (millis() - lastTransmitTime >= 20) {
    lastTransmitTime = millis();

    Serial.print(xMM, 3);
    Serial.print(',');
    Serial.println(yMM, 3);
  }

  /*PMW3389 breakout          Arduino Mega
  ---------------------------------------
  VCC / 3V3   ----------->  3.3V
  GND         ----------->  GND
  MISO / SDO  ----------->  Pin 50
  MOSI / SDI  ----------->  Pin 51
  SCLK / SCK  ----------->  Pin 52
  CS / NCS    ----------->  Pin 53
  MOTION      ----------->  Pin 2 */
 

  delay(1);
}
